...
WITH (
    ...
    'properties.group.id' = 'nexi-flink-processing',
...
',
    ...
    'properties.sasl.jaas.config' = 'org.apache.kafka.common.security.scram.ScramLoginModule required username="ts" password="H6Rg98HBOdaOn7ZSRv6QYQJDhXKfRyt7";'
);
...
FROM `10Z JOIN 12Z` JOIN `13Z To Timestamps` ON
    `10Z JOIN 12Z`.`S_PREF_PK_OP_10` LIKE `13Z To Timestamps`.`S_PREF_PK_OP`
...
SELECT
 ...
 AS `E_TX_MSG_OUT_12_MICROSECONDS`,
    ...
    CAST(IF (C_APPRVL  IS NULL, IF(C_ST_RCH_GT   = '1', 'TimeOut', IF(C_ST_RCH_GT   = '2' AND C_ST_RCH_EMTT = '2', 'Negata Issuer', IF(C_ST_RCH_GT   = '2' AND C_ST_RCH_EMTT IS NULL, 'Negata Interna', 'AUTORIZZATA'))), 'AUTORIZZATA')
 AS STRING) AS `ESITO`,

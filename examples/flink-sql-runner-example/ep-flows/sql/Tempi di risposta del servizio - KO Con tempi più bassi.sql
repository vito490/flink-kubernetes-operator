...
-- exportedDate: 2024-04-04T16:04:13.110+0000
...
FROM TABLE (
    TUMBLE( TABLE `Transazioni KO`, DESCRIPTOR(`event_time`), INTERVAL '1' SECOND )
)
)
...
FROM TABLE (
    TUMBLE( TABLE `OK Tempi più bassi`, DESCRIPTOR(`event_time`), INTERVAL '1' SECOND )
)
GROUP BY
    `window_start`,
    `window_end`,

...
-- exportedDate: 2024-04-04T16:04:21.560+0000
...
FROM TABLE (
    TUMBLE( TABLE `source_1`, DESCRIPTOR(`event_time`), INTERVAL '1' SECOND )
)
GROUP BY
    `window_start`,
    `window_end`,
...
INSERT INTO `sink_1` SELECT * FROM `Valore transazioni medio e totale`;
-- -------------------------------------------

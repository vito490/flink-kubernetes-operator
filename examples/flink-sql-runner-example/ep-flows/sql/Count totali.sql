...
-- exportedDate: 2024-04-04T13:47:35.212+0000
...
FROM TABLE (
    TUMBLE( TABLE `Approvate`, DESCRIPTOR(`event_time`), INTERVAL '5' SECOND )
)
GROUP BY
    `window_start`,
    `window_end`,
...
FROM TABLE (
    TUMBLE( TABLE `Non Approvate`, DESCRIPTOR(`event_time`), INTERVAL '5' SECOND )
)
GROUP BY
    `window_start`,
    `window_end`,
...
WHERE `Somma Approvate`.`aggregateResultTime`
    BETWEEN `Somma Non Approvate`.`aggregateResultTime` - INTERVAL '5' SECOND
    AND `Somma Non Approvate`.`aggregateResultTime` + INTERVAL '5' SECOND;
